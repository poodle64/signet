...
signet speaks the broker attestation HTTP contract and only that contract: `POST /v1/attest/challenge`, `POST /v1/attest/token`, and `POST /v1/attest/renew` (the three legs in `internal/attest/attest.go`). The whole flow is three requests and a signature in between; there is no fourth endpoint, no sidecar, no helper process, no PKCS#11 module. A new broker capability does not earn signet a new code path unless it lands on this `/v1/attest/*` surface — with one deliberate, bounded exception: the consumer-preflight tools (`verify`, since 2026.7.0), the vend-to-headers helper (`headers`, since 2026.7.1), the vend-to-file helper (`vend-to-file`, since 2026.7.2), and the vend-and-exec helper (`exec`, since 2026.7.3) also call `GET /v1/credentials/{name}`, the broker's public consumer vend door, authenticated by the bearer the attest legs mint. That is the broker's stable consumer contract, not a private surface; signet still never touches any other endpoint family.
